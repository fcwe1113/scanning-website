...
const Login = () => {
    ...
    return (
        <>
            ...
            {/* <div id="button_div"> */}
            <button onClick={() => LoginHandler()}>Sign in</button><br />
...
function LoginHandler(){
    ...
    const usernameInput = (document.getElementById("username_input") as HTMLInputElement).value
    const passwordInput = (document.getElementById("password_input") as HTMLInputElement).value

    // do some input sanitising before passing along the inputs
    // add more here later if needed
    if(usernameInput.length > 0){
        if(passwordInput.length > 0){
            socket.send(nonce.value + "1LOGIN" + usernameInput + " " + passwordInput)
            console.debug("sent: " + nonce.value + "1LOGIN" + usernameInput + " " + passwordInput)
